...
def main(r=0):
    ...
    for ndim, deg in ndim_deg_cases:
        ...
        filename_test   = lambda r: r'CDF_McsE7R{:d}'.format(r)
        ...
        pf_pred = np.sum(data_pred.y < 0)/len(data_pred.y)
        print(uqra.metrics.mquantiles(data_pred.y, 1-pf))
        ...
        for idoe_sampling, ioptimality in all_doe_cases:
            ...
            data_temp.pf_hat.append(np.sum(y_pred_hat<0)/len(y_pred_hat))
            print('      pf test [PCE]  : {:.4f}'.format(np.sum(y_test_hat<0)/len(y_test_hat)))
            print('      pf pred [PCE]  : {:.4f}'.format(np.sum(y_pred_hat<0)/len(y_pred_hat)))
            ...
            is_converge, pf_hat, pf_hat_err = check_converge(data_temp.pf_hat, abs_tol=1e-3)
            ...
            print(uqra.metrics.mquantiles(y_pred_hat, 1-pf))
            print(uqra.metrics.mquantiles(data_pred.y, 1-pf))
        ...
        is_converge, pf_hat, pf_hat_err = check_converge(deg_stop_pf_hat, abs_tol=1e-3)
