...
def get_test_data(simparams, solver, doe_method, n_test, filename_z = r'DoE_McsE6R0.npy'):
    """
    Return test data. 
    ...
    if solver.nickname.lower().startswith('poly') or solver.nickname.lower().startswith('sparsepoly'):
        filename_result = filename_z[:-4]+'_{:s}{:d}_p{:d}.npy'.format(solver.basis.nickname, solver.ndim,solver.basis.deg)
    else:
        filename_result = filename_z
    ...
    try:
        ...
        if solver.dist_name.lower() == 'uniform':
            print('   > Solving test data from {} '.format(os.path.join(simparams.data_dir_sample, 'MCS','Uniform', filename_z)))
            data_set = np.load(os.path.join(simparams.data_dir_sample, 'MCS','Uniform', filename_z))
            ...
            print('   > Solving test data from {} '.format(os.path.join(simparams.data_dir_sample, 'MCS','Normal', filename_z)))
            data_set = np.load(os.path.join(simparams.data_dir_sample, 'MCS','Normal', filename_z))
            z_test = data_set[:solver.ndim,:] 
