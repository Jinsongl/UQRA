...
def main():
    ...
    # for r in tqdm(range(1)):
        # doe = museuq.LHS(n_samples=195,dist_names=['normal']*ndim,ndim=ndim,dist_theta=[(0,1)]*ndim)
        # u_doe, x_doe = doe.samples() ## u in [0,1], x in N(0,1)
        # temp = x_doe
        # x_doe = Kvitebjorn.samples(u_doe)
        # u_doe, x_doe = x_doe, Kvitebjorn.samples(u_doe)
        # data = np.concatenate((u_doe, x_doe), axis=0)
        # filename = os.path.join(simparams.data_dir, doe.filename+'R{:d}'.format(r))
        # np.save(filename, data)

    # filename = 'DoE_Lhs195.npy'
    # data_set = np.load(os.path.join(simparams.data_dir, filename))
    # u_train  = data_set[0:ndim,:] 
    # x_train  = data_set[ndim:2*ndim,:] 
    # y_train  = np.array([solver.run(x_train)[1] for _ in range(n_sim_short)])
    # filename = 'DoE_Lhs195_y.npy'
    # filename = np.save(os.path.join(simparams.data_dir, filename), y_train)
    ## 3. Quadrature Design
    quad_orders = range(13,14)
    for iquad_orders in tqdm(quad_orders):
        ...
        w_train  = data_set[-2,:]
        y_train  = np.array([solver.run(x_train)[1] for _ in range(n_sim_short)])
        filename = 'DoE_QuadHem{:d}_y.npy'.format(iquad_orders)
        filename = np.save(os.path.join(data_dir, filename), y_train)
