...
def main():
    ...
    for iquad_orders in quad_orders:
        ...
        doe.x = Kvitebjorn.samples(np.array([dist_normal.cdf(doe.u[0,:]), dist_normal.cdf(doe.u[1,:])]))
        solver.run(doe.x)
        print(solver.y.shape)
        doe_data = np.concatenate((doe.u, doe.x, doe.w.reshape(1,-1),np.squeeze(solver.y[:,4,:]).T), axis=0)
        filename = os.path.join(simparams.data_dir, doe.filename)
        np.save(filename, doe_data)
        filename = os.path.join(simparams.data_dir, doe.filename+'_stats')
        np.save(filename, solver.y)
