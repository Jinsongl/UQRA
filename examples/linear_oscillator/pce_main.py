...
def main():
    ...
    for iquad_orders in quad_orders:
        ...
        data_set = np.load(os.path.join(simparams.data_dir,'DoE_QuadHem_PCE_OLS_GLK', filename))
        print('  > {:<10s}: {:s}'.format('filename', filename))
        train_u  = data_set[0:2,:] 
        train_x  = data_set[2:4,:] 
        train_w  = data_set[4,:]
        train_eta= data_set[5,:]
        train_y  = data_set[6,:]
        ...
        for r in range(10):
            ...
            data_set = np.load(os.path.join(simparams.data_dir, 'DoE_McsE6', filename))
            valid_u  = data_set[0:2,:]
            valid_x  = data_set[2:4,:]

            filename = 'DoE_McsE6R{:d}_stats.npy'.format(r)
            data_set = np.load(os.path.join(simparams.data_dir, 'DoE_McsE6', filename))
            valid_eta= np.squeeze(data_set[:,4,0])
            valid_y  = np.squeeze(data_set[:,4,1])
            ...
            # filename = 'DoE_QuadHem{:d}_{:s}_{:s}_E6R{:d}_y_score.npy'.format(iquad_orders, metamodel_class, pce_fit_method, r)
            # np.save(os.path.join(simparams.data_dir, filename), pce_valid_score)

            pce_valid_y, pce_valid_score = pce_model.predict(valid_u, valid_eta, metrics=metrics2cal, prob=upper_tail_probs, moment=moment2cal)
            pce_valid_y_ecdf = uqhelpers.get_exceedance_data(pce_valid_y, 1e-5)
            filename = 'DoE_QuadHem{:d}_{:s}_{:s}_E6R{:d}_eta.npy'.format(iquad_orders, metamodel_class, pce_fit_method, r)
            np.save(os.path.join(simparams.data_dir, filename), pce_valid_y)

            filename = 'DoE_QuadHem{:d}_{:s}_{:s}_E6R{:d}_eta_ecdf.npy'.format(iquad_orders, metamodel_class, pce_fit_method, r)
            np.save(os.path.join(simparams.data_dir, filename), pce_valid_y_ecdf)

            filename = 'DoE_QuadHem{:d}_{:s}_{:s}_E6R{:d}_eta_score.npy'.format(iquad_orders, metamodel_class, pce_fit_method, r)
            np.save(os.path.join(simparams.data_dir, filename), pce_valid_score)
    ...
    # for iquad_orders in quad_orders:
        ...
        # for r in range(10):
            ...
            # for ia in alpha:
                ...
                # filename = 'DoE_McsE6R{:d}_q{:d}_Opt{:s}{:d}_{:s}_{:s}_y.npy'.format(r, iquad_orders, opt_cri, num_basis,metamodel_class, pce_fit_method, r)
                # np.save(os.path.join(simparams.data_dir, filename), pce_valid_y)

                # filename = 'DoE_McsE6R{:d}_q{:d}_Opt{:s}{:d}_{:s}_{:s}_y_ecdf.npy'.format(r, iquad_orders, opt_cri, num_basis,metamodel_class, pce_fit_method, r)
                # np.save(os.path.join(simparams.data_dir, filename), pce_valid_y_ecdf)

                # filename = 'DoE_McsE6R{:d}_q{:d}_Opt{:s}{:d}_{:s}_{:s}_score.npy'.format(r, iquad_orders, opt_cri, num_basis,metamodel_class, pce_fit_method, r)
                # np.save(os.path.join(simparams.data_dir, filename), pce_valid_score)
                ...
                # filename = 'DoE_McsE6R{:d}_q{:d}_Opt{:s}{:d}_{:s}_{:s}_eta.npy'.format(r, iquad_orders, opt_cri, num_basis,metamodel_class, pce_fit_method, r)
                # np.save(os.path.join(simparams.data_dir, filename), pce_valid_y)

                # filename = 'DoE_McsE6R{:d}_q{:d}_Opt{:s}{:d}_{:s}_{:s}_eta_ecdf.npy'.format(r, iquad_orders, opt_cri, num_basis,metamodel_class, pce_fit_method, r)
                # np.save(os.path.join(simparams.data_dir, filename), pce_valid_y_ecdf)

                # filename = 'DoE_McsE6R{:d}_q{:d}_Opt{:s}{:d}_{:s}_{:s}_eta_score.npy'.format(r, iquad_orders, opt_cri, num_basis,metamodel_class, pce_fit_method, r)
                # np.save(os.path.join(simparams.data_dir, filename), pce_valid_score)
