...
def main(ST):
    ...
    for pce_deg in simparams.pce_degs:
        ...
        n_new_train = max(len(pce_model.var_pct_basis), 2*len(pce_model.var_pct_basis) - u_train.shape[1])
        tqdm.write('    > {}:{}; Basis: {}/{}; #samples = {:d}'.format(
            'Sampling', simparams.optimality, len(pce_model.var_pct_basis), pce_model.num_basis, n_new_train ))
        u_train_new, _ = modeling.get_train_data(n_new_train, u_cand, u_train, basis=pce_model.basis, 
        ...
        data_test.append([pce_deg, u_train.shape[1], y_pred])
        res = [pce_deg, u_train.shape[1], pce_model.cv_error, test_error[0]]
