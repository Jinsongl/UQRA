...
def main(theta):
    # print('------------------------------------------------------------')
    # print('>>> Repeated Simu (n={:d})  '.format(theta))
    # print('------------------------------------------------------------')
    ...
    filename    = 'Franke_2Leg_DoE_McsE6R{:d}.npy'.format(theta)
    ...
    for deg in simparams.pce_degs:
        ...
        if simparams.optimality is None:
            ...
            filename = 'DoE_{:s}E5R{:d}_{:s}_{:s}.npy'.format(simparams.doe_method.capitalize(), theta,
