...
def main(model_params, doe_params, solver, r=0, random_state=None):
    ...
    for i, (ndim, deg) in enumerate(ndim_deg_cases):
        ...
        while True:
            ...
            n_samples    = sparsity #min(sparsity, model_params.alpha *pce_model.num_basis - n_samples_deg, 5)
            print('                 ------------------------------')
            ...
            data_ideg.y_train_.append (y_train_)
            n_samples_deg = len(np.concatenate(data_ideg.y_train_, axis=0))
            # if n_samples_deg > math.ceil(model_params.alpha * pce_model.num_basis):
                # ## pop out the last element
                # data_ideg.xi_train_.pop()
                # data_ideg.x_train_.pop()
                # data_ideg.y_train_.pop()
                # print('         !< Number of samples exceeding {}xP >!'.format(model_params.alpha))
                # break
