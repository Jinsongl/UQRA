...
def main():
    ...
    ### ============ Get design points ============
    filename= 'DoE_McsE6R0_q15_OptD600.npy'
    ...
    filename = 'mquantile_DoE_q15_OptD600_PCE{:d}_{:s}_path.npy'.format(poly_order, fit_method)
    np.save(os.path.join(simparams.data_dir, filename), np.array(mquantiles))

    filename = 'r2_DoE_q15_OptD600_PCE{:d}_{:s}_path.npy'.format(poly_order, fit_method)
    np.save(os.path.join(simparams.data_dir, filename), np.array(r2_score_adj))

    filename = 'cv_error_DoE_q15_OptD600_PCE{:d}_{:s}_path.npy'.format(poly_order, fit_method)
    ...
    for r in tqdm(range(10), ascii=True, desc="   - " ):
        ...
        filename = 'DoE_McsE6R{:d}_q15_OptD600_PCE{:d}_{:s}.npy'.format(r, poly_order, fit_method)
        np.save(os.path.join(simparams.data_dir, filename), y_samples)

    filename = 'mquantile_DoE_q15_OptD600_PCE{:d}_{:s}.npy'.format(poly_order, fit_method)
    np.save(os.path.join(simparams.data_dir, filename), np.array(mquantiles))
