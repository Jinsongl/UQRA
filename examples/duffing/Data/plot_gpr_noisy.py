...
y = 0.5 * np.sin(3 * X[:, 0]) + X[:, 0]**2+ rng.normal(0, 0.5, X.shape[0])
...
plt.plot(X_, y_mean, 'k', lw=3, zorder=9,label=r'pred_mean')
plt.fill_between(X_, y_mean - np.sqrt(np.diag(y_cov)),
                 y_mean + np.sqrt(np.diag(y_cov)),
                 alpha=0.5, color='k')
plt.plot(X_, 0.5*np.sin(3*X_+X_**2), 'r', lw=3, zorder=9,label=r'true')
