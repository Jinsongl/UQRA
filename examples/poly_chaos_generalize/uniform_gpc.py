...
def _pce_approx(dist_x, dist_zeta, npoly_order, nquad=150, rule=None):
    ...
    print('zeta_quad:{}'.format(zeta_quad))
    print('zeta_weight:{}'.format(zeta_weight))
    print('x_quad:{}'.format(x_quad))
...
for ipoly_order in npoly_order:
    ...
    # _x2_hat, _x2_coeffs = _pce_approx(dist_x, dist_zeta2, ipoly_order, rule='e')
    _x2_hat, _x2_coeffs = _pce_approx(dist_x, dist_zeta2, ipoly_order, rule='jacobi')
...
axes[0,0].plot(np.arange(npoly_order[-1]+1), x2_coeffs[-1],'-s',color=pltcolors[1], label=r'Jacobi-chaos')
...
axes[1,1].plot(npoly_order, x2_stats_hat[:,4],linestyle=pltlinestyles[2],color=pltcolors[1], marker=pltmarkers[2], label=r'$\alpha={}$(Jacobi-chaos)'.format(1-p1))
...
axes[1,1].plot(npoly_order, x2_stats_hat[:,5],linestyle=pltlinestyles[3],color=pltcolors[1], marker=pltmarkers[3], label=r'$\alpha={}$(Jacobi-chaos)'.format(1-p2))
...
plt.savefig('Figures/uniform_pce.eps')
plt.savefig('Figures/uniform_pce.png')
