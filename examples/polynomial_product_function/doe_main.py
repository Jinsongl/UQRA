...
def main():
    ## ------------------------ Parameters set-up ----------------------- ###
    model_name  = 'polynomial_product_function'
    ndim        = 10
    ...
    for r in tqdm(range(10)):
        doe = museuq.RandomDesign('MCS', n_samples=1e6, ndim=ndim, dist_names= ['normal',]*ndim, dist_theta=[(0,1),]*ndim)
        doe.samples()
        doe.x = doe.u
        solver.run(doe.x)
    ...
    # for iquad_orders in quad_orders:
        ...
        # for r in range(4):
            ...
            # samples_y = data_set[4,:].reshape(1,-1)
