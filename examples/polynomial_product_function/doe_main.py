...
def main():
    ...
    # for r in tqdm(range(10)):
        # doe = uqra.RandomDesign('MCS', n_samples=n_samples, ndim=ndim, dist_names= ['normal',]*ndim, dist_theta=[(0,1),]*ndim)
        # u_doe = doe.samples()
        # x_doe = u_doe 
    ...
    for r in tqdm(range(1)):
        doe = uqra.LHS(n_samples=38257,dist_names=['normal']*ndim,ndim=ndim,dist_theta=[(0,1)]*ndim)
    ...
    # for iquad_orders in tqdm(quad_orders):
        # doe = uqra.QuadratureDesign(iquad_orders, ndim = ndim, dist_names=['normal', ] *ndim)
