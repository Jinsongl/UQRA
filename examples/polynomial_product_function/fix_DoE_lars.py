...
def main():
    ...
    ### ============ Get training points ============
    filename= 'DoE_LhsE3R0_ndim{:d}.npy'.format(ndim)
    ...
    filename = 'mquantile_DoE_LhsE3_ndim{:d}_PCE{:d}_{:s}_path.npy'.format(ndim, poly_order, fit_method)
    np.save(os.path.join(simparams.data_dir, filename), np.array(mquantiles))

    filename = 'r2_DoE_LhsE3_ndim{:d}_PCE{:d}_{:s}_path.npy'.format(ndim, poly_order, fit_method)
    ...
    filename = 'mquantile_DoE_LhsE3_ndim{:d}_PCE{:d}_{:s}.npy'.format(ndim, poly_order, fit_method)
    np.save(os.path.join(simparams.data_dir, filename), np.array(mquantiles))
