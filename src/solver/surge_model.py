...
class surge_model(SolverBase):
    ...
    def _linear_oscillator(self, x, random_seed=None, ret_raw=False, out_responses='ALL'):
        ...
        if 0 in out_responses:
            ...
            istats = uqra.get_stats(t0, out_stats=self.out_stats, axis=0)
        ...
        if 1 in out_responses:
            ...
            istats  = uqra.get_stats(eta, out_stats=self.out_stats, axis=0)
            y_stats.append(istats)
            if ret_raw:
        ...
        if 2 in out_responses:
            ...
            istats  = uqra.get_stats(x_t, out_stats=self.out_stats, axis=0)
            y_stats.append(istats)
            if ret_raw:
        ...
        if 3 in out_responses:
            ...
            istats  = uqra.get_stats(y_t, out_stats=self.out_stats, axis=0)
            y_stats.append(istats)
            if ret_raw:
        ...
        uqra.enablePrint()
        # y_stats = uqra.get_stats(y_raw, out_responses=out_responses, out_stats=self.out_stats, axis=0) 
        if ret_raw:
